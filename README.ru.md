...
* FuryDumper::Dumper::Model - класс модели
* FuryDumper::Dumper::ModelQueue - очередь моделей для дампа в ширину
* FuryDumper::Dumper::DumpState - класс состояния дампа, тут хранится информация о тех моделях, что уже дампали и подводится небольшая статистика по дампу
* FuryDumper::Dumper::RelationItem - структура связи - ключи и значения, по которым дампали. Для обычных моделей в RelationItem сравнивается между собой только по ключам. Additional опция дает возможность сравнить по ключу и значению. Complex - явно говорит о том, что будет только ключ, то есть в ключе содержится строка типа `date_from IS NULL`, являющаяся условием для связи. 
